...
function compare(a, b) {
  const published_atA = a.published_at;
  const published_atB = b.published_at;

  let comparison = 0;
  if (published_atA > published_atB) {
    comparison = 1;
  } else if (published_atA < published_atB) {
    comparison = -1;
  }
  return comparison;
}

print(JSON.stringify(data.sort(compare)))
